...
try{await start();
await test('default admin login is gated until the password is changed',async()=>{
 const status=await(await call('auth/status','GET',undefined,{})).json();assert.equal(status.authenticated,false);assert.equal(status.passwordConfigured,true);
 const login=await call('auth/login','POST',{username:'admin',password:'123456'},{});assert.equal(login.status,200);
 const cookie=login.headers.get('set-cookie').split(';')[0];
 const current=await(await call('auth/status','GET',undefined,{Cookie:cookie})).json();assert.equal(current.mustChangePassword,true);
 assert.equal((await call('servers','GET',undefined,{Cookie:cookie})).status,403);
 assert.equal((await call('settings','GET',undefined,{Cookie:cookie})).status,403);
 assert.equal((await call('servers','GET',undefined,{Cookie:cookie,Authorization:'Bearer invalid'})).status,403);
 assert.equal((await call('auth/logout','POST',undefined,{Cookie:cookie})).status,200);
});
...
await test('HTTP password change revokes sessions, replacement login and logout revoke access',async()=>{
 ...
 assert.equal((await call('auth/password','POST',{username:'operator',password:'changed-vps-password-123',currentPassword:'vps-test-password-123'},{Cookie:cookie})).status,200);
 ...
 await stop();await start();
 const replacement=await call('auth/login','POST',{username:'operator',password:'changed-vps-password-123'},{});assert.equal(replacement.status,200);
 assert.equal((await call('auth/login','POST',{username:'admin',password:'123456'},{})).status,401);
