...
export type Sample={time:number;cpu:number;memory:number;disk:number;upload:number;download:number};
...
export function sampleOf(s:Server):Sample|null{const m=s.metrics;return m&&s.seen?{time:s.seen,cpu:m.cpu,memory:m.memory,disk:m.disk,upload:m.upload,download:m.download}:null}
