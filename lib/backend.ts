...
export function telegramMessage(e:any,s:any,analysis='',platformName='全球VPS联动观察'){const title=e.server==='test'?'🧪 通知连接测试':e.kind==='recovery'?'✅ 状态恢复':e.kind==='warning'?'🚨 服务器告警':'🔔 状态通知';if(!s)return [title,`🏷 平台：${platformName}`,'━━━━━━━━━━━━━━━━━━',`📣 通知内容\n${e.message}`,analysis?'🤖 健康分析\n'+analysis:'','━━━━━━━━━━━━━━━━━━',`🕒 ${new Date(e.time*1000).toISOString().replace('T',' ').slice(0,19)} UTC`].filter(Boolean).join('\n\n').slice(0,4000);const m=s.metrics,meta=s.meta;return [`${title} · ${platformName}`, '━━━━━━━━━━━━━━━━━━',`${flag(meta.country)} ${meta.name}`,`📍 ${countryName(meta.country)} · ${meta.region}`,`🌐 ${meta.ip||'IP 待识别'}  ${meta.asn||''}`,`🏢 ${meta.operator} · ${meta.network}`,`\n📣 事件详情\n${e.message}`, '\n━━━━━━━━━━━━━━━━━━\n📈 实时资源',m?`🖥 系统：${meta.osOverride||m.os} · ${m.arch}`:'🖥 等待首个采样',...(m?[`⚙️ CPU：${m.cpu.toFixed(1)}% · ${m.cores} 核 · 负载 ${m.load.toFixed(2)}`,`🧠 内存：${m.memory.toFixed(1)}% · ${bytes(m.memoryTotal*m.memory/100)} / ${bytes(m.memoryTotal)}`,`💾 根磁盘：${m.disk.toFixed(1)}% · ${bytes(m.diskTotal*m.disk/100)} / ${bytes(m.diskTotal)}`,`♻️ Swap：${m.swap.toFixed(1)}%`,...(m.disks||[]).map((d:any)=>`📂 ${d.path}：${d.percent.toFixed(1)}% · ${bytes(d.used)} / ${bytes(d.total)}`),'\n🌐 网络与连接',`⬆️ 上传：${bytes(m.upload)}/s   ⬇️ 下载：${bytes(m.download)}/s`,`📊 周期流量：${bytes(trafficUsed(s))} / ${unlimited(meta)?'无限':meta.quota+' GB'}`,`🔌 TCP ${m.tcp} · UDP ${m.udp}`,`⏱ 系统运行：${duration(m.uptime)}`,...m.checks.map((p:any)=>`📡 ${p.name}：${p.ms===null?'连接失败':p.ms.toFixed(0)+' ms'} · 失败率 ${p.loss}%`)]:[]),'\n━━━━━━━━━━━━━━━━━━\n🗓 套餐与到期',`💳 ${planLabel(meta)} · 📅 ${expiryLabel(meta)}`,meta.note?'📝 '+meta.note:'',analysis?'\n🤖 健康分析\n'+analysis:'',`🕒 ${new Date(e.time*1000).toISOString().replace('T',' ').slice(0,19)} UTC`].filter(Boolean).join('\n').slice(0,4000)}
