...
import {profileApi,profileData} from './profile';
...
export function telegramMessage(e:any,s:any,_analysis='',platformName='全球VPS联动观察'){
 const offline=e.message.includes('离线'),title=e.server==='test'?'🧪 通知连接测试':e.kind==='recovery'?(offline?'🟢 服务器重新上线':'✅ 资源状态恢复'):e.kind==='warning'?(offline?'🔴 服务器离线':'🚨 资源 / 到期告警'):'🔔 状态变化';
 const message=String(e.message).replace(/ · cpu/g,' · CPU').replace(/ · memory/g,' · 内存').replace(/ · disk/g,' · 磁盘').replace(/ · swap/g,' · Swap');
 return [title,`🏷 ${platformName}`,'━━━━━━━━━━━━━━━━━━',s?`${flag(s.meta.country)} 服务器：${s.meta.name}`:'🧪 平台通知',s?`🌐 IP：${s.meta.ip||'待识别'}`:'',`📣 事件：${message}`,`🕒 ${new Date(e.time*1000).toISOString().replace('T',' ').slice(0,19)} UTC`,e.id?`🧾 记录：${e.id}`:''].filter(Boolean).join('\n').slice(0,4000)
}
...
export async function evaluate(row:any,m:any,c:any,now:number){
 for(const key of ['cpu','memory','disk','swap']){const a:any=await db().prepare('SELECT active FROM alerts WHERE id=?').bind(`${row.id}:${key}`).first();await transition(row,key,a?.active?m[key]>Math.max(0,c[key]-c.recovery):m[key]>=c[key],`${m[key].toFixed(1)}% / 阈值 ${c[key]}%`,m[key]>=100?{...c,hold:0}:c,now)}
 for(const d of m.disks||[]){if(d.path==='/')continue;const key=`磁盘 ${d.path}`,a:any=await db().prepare('SELECT active FROM alerts WHERE id=?').bind(`${row.id}:${key}`).first();await transition(row,key,a?.active?d.percent>Math.max(0,c.disk-c.recovery):d.percent>=c.disk,`${d.percent.toFixed(1)}% / 阈值 ${c.disk}%`,d.percent>=100?{...c,hold:0}:c,now)}
 const s={...publicServer(row),metrics:m};if(!unlimited(s.meta)&&s.meta.trafficMode!=='unknown'&&s.meta.quota>0){const a:any=await db().prepare('SELECT active FROM alerts WHERE id=?').bind(row.id+':流量').first(),pct=trafficUsed(s)/1024**3/s.meta.quota*100;await transition(row,'流量',a?.active?pct>Math.max(0,c.traffic-c.recovery):pct>=c.traffic,`${(trafficUsed(s)/1024**3).toFixed(2)} GB / ${s.meta.quota} GB · 阈值 ${c.traffic}%`,c,now)}else await db().prepare('UPDATE alerts SET active=0,since=0 WHERE id=?').bind(row.id+':流量').run();
 // Probe delay/failure remains chart data, never a Telegram resource incident.
 await transition(row,'离线',false,'已收到探针心跳',{...c,hold:0},now)
}
export async function deliver(o: string) { const c = await config(o); if (!c.telegramEnabled || !c.telegramToken || !c.telegramChat)
    ...
    if(/ · .*?(?:延迟|丢包)(?:告警|已恢复)：/.test(e.message)){await db().prepare('UPDATE events SET delivered=2 WHERE id=? AND delivered=0').bind(e.id).run();continue}
    ...
    try {
        ...
        const explanation='';
        const r = await fetch(`https://api.telegram.org/bot${c.telegramToken}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: c.telegramChat, text: telegramMessage(e,snapshot,explanation,(await profileData(db(),o)).platformName) }), signal: AbortSignal.timeout(12000) });
...
export async function api(r: Request, path: string[]) {
    ...
    if (route === 'report' && r.method === 'POST') {
        ...
        Object.assign(m,!previous&&meta.trafficCalibration?.cycle===cycle?{cycle,cycleTx:0,cycleRx:0,trafficBasis:'校准后采样'}:accumulateTraffic(m,previous&&(meta.trafficCalibration?.cycle===cycle||meta.trafficOffsetGb>0)?{...previous,trafficBasis:previous.trafficBasis||'已补录采样'}:previous,cycle,now));
    ...
    if (path[0] === 'servers' && path[1]) {
        ...
        if(path[2]==='traffic'&&r.method==='PUT'){
            const data=z.object({usedGb:num.max(1e9)}).parse(await r.json()),meta=JSON.parse(row.meta),m=row.metrics?JSON.parse(row.metrics):null;
            meta.trafficCalibration={cycle:cycleKey(meta.resetDay),usedGb:data.usedGb,baseline:(m?.cycle===cycleKey(meta.resetDay)?(m.cycleTx||0)+(m.cycleRx||0):0),at:now};meta.trafficOffsetGb=0;
            await db().prepare('UPDATE servers SET meta=? WHERE id=? AND owner=?').bind(JSON.stringify(meta),row.id,o).run();await auditRecord(db(),o,'billing','校准本周期流量',JSON.stringify({name:meta.name,note:`服务商已用量 ${data.usedGb} GB；后续自动叠加新采样`} ),row.id);return Response.json({ok:true});
        }
    ...
    if (route === 'settings' && r.method === 'GET') {
        ...
        return Response.json({ ...c, ipinfoToken: '', hasTelegramToken: !!c.telegramToken, hasAiKey: !!c.aiKey, hasIpinfoToken: !!c.ipinfoToken }, { headers: { 'Cache-Control': 'no-store' } });
    }
