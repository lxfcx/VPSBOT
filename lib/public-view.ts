...
export function viewerServer(row:any):Server{
 ...
 let metrics:any=null;if(row.metrics){const m=JSON.parse(row.metrics);metrics={};for(const key of ['cpuModel','virtualization','processes','kernel','cpu','memory','disk','swap','load','cores','memoryTotal','diskTotal','upload','download','tx','rx','tcp','udp','uptime','os','arch','cycle','cycleTx','cycleRx','trafficBasis'])if(m[key]!==undefined)metrics[key]=m[key];metrics.checks=(m.checks||[]).map((p:any,i:number)=>({name:p.carrier?({telecom:'中国电信',unicom:'中国联通',mobile:'中国移动'} as any)[p.carrier]||'线路检测':['Google','Cloudflare','Apple'].includes(p.name)?p.name:`线路 ${i+1}`,ms:p.ms,loss:p.loss,carrier:p.carrier}));}
